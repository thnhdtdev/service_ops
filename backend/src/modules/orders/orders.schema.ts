...
export const createOrderSchema = z
  .object({
    ...
    paid_amount: z
      .number()
      .int({
        message: "Số tiền thanh toán phải là số nguyên",
      })
      .min(0)
      .optional()
      .default(0),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.paid_amount > 0 && !data.payment_method) {
      ctx.addIssue({
        code: "custom",
        path: ["payment_method"],
        message: "Phải chọn phương thức thanh toán khi khách có thanh toán",
      });
    }
    ...
    if (data.discount_value === 0 && data.discount_type != null) {
      ctx.addIssue({
        code: "custom",
        path: ["discount_type"],
        message:
    ...
    if (data.discount_value > 0 && data.discount_type == null) {
      ctx.addIssue({
        code: "custom",
        path: ["discount_type"],
        message: "Phải chọn loại chiết khấu khi giá trị chiết khấu lớn hơn 0",
      });
    }
    ...
    if (data.discount_type === "percent" && data.discount_value >= 100) {
      ctx.addIssue({
        code: "custom",
        path: ["discount_value"],
        message: "Chiết khấu phần trăm phải nhỏ hơn 100%",
      });
    }
  });
...
export type GetOrdersInput = z.infer<typeof getOrdersQuerySchema>;
